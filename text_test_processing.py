...
action_words = {'навигация': ['сдела', 'добав', 'посмотрет', 'глянут', 'узна',
                              'отслед', 'перейт', 'провер', 'откр'],
                'заполнение': ['заполня', 'внос', 'написа', 'вписа', 'внест', 'добав',
                               'нужн', 'описа', 'писа', 'вписа', 'записа']}
words_to_section = {'добавить проект': ['созда', 'сдела', 'описа', 'добав'],
               ...
               'мои заявки': ['заявк', 'провер', 'посмотрет', 'откр', 'просмотрет', 'статус', 'глянут', 'узна', 'отслед', 'перейт'],
                ...
                'база знаний': ['файл', 'шаблон', 'cтат', 'документ', 'грант']}
detect_faq = {'обратная связь по гранту': ['обратн', 'связ', 'узна', 'грант', 'вопрос', 'информац'],
       'обновление данных профиля ГАИС': ['обновлен', 'дан', 'госуслуг', 'гаис', 'информац'],
       'перезайти ЕСИА': ['перезайт', 'обнов', 'ес'],
       'не сохраняются изменения в профиле': ['сохраня', 'изменен'],
       ...
       'узнать id профиля': ['узна', 'id', 'посмотрет'],
       ...
       'удаление лк': ['удал', 'личн', 'кабинет', 'лк', 'профил']}
        ...
        for key in detect_doc_fields:
            ...
            for word in words:
                if word in detect_doc_fields[key]:
                    count_field += 1
            if count_field >= porog_count:
                # dict_answer[key] = field
                # field = key
                print(show_field_info(key))
                return show_field_info(key)
        if not mas_answer:
            return 'Я не смогла найти ответ на Ваш вопрос. Попробуйте перефразировать его или ' \
                   'отправьте его в тех.поддержку по адресу support@myrosmol.ru'
        else:
        ...
        for key in detect_faq:
            ...
            for word in words:
                if word in detect_faq[key]:
                    count_faq += 1
            if count_faq >= porog_count_faq:
                print(answer_faq(key))
                return answer_faq(key)
...
def web_bot(text):
    ...
    return func(choose_action(words), words)
...
'''def main():
    ...
    mas = []
    return mas'''
web_bot('Открыть раздел архив проектов')
#web_bot('Архив арбузы')
