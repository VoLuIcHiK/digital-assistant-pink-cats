...
words_to_section = {'добавить проект': ['созда', 'проект', 'сдела', 'описа', 'добав'],
               ...
               'мои заявки': ['заявк', 'провер', 'посмотрет', 'откр', 'просмотрет', 'статус', 'глянут', 'узна', 'отслед'],
                    'архив проектов': ['проект', 'сохранен', 'реализова', 'созда', 'посмотрет', 'глянут', 'архив',
                                       'прошл', 'предыдущ'],
                'мероприятия': ['нов', 'мероприят', 'событ', 'форум', 'встреч', 'событ'],
...
def func(words, len_text):
    '''Функция, отвечающая за генерацию ответа на вопрос, где
...
def web_bot(text):
    ...
    return func(words, len(text.split()))
...
'''def main():
    ...
    mas = []
    return mas'''
#web_bot('Что писать в опыт руководителя')
#web_bot('Как выбирать регион')